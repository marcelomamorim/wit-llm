...
func construirPromptGeracaoSistema(framework string) string {
	...
	case frameworkJTReg:
		return "Você é um especialista em escrita de testes Java para o OpenJDK usando jtreg. Responda apenas com JSON. Gere testes standalone compatíveis com jtreg, preferencialmente com bloco @test, @summary, @modules quando necessário e @run main. Não use JUnit, Mockito, AssertJ ou bibliotecas externas. Para jtreg no OpenJDK, não declare package no conteúdo do arquivo gerado: mantenha o teste no pacote padrão, mesmo quando o alvo estiver em java.*, javax.*, sun.* ou com.sun.*. Use @modules para declarar acesso a módulos/pacotes internos quando necessário, e prefira APIs públicas ou caminhos observáveis. Use apenas APIs visíveis no checkout atual e classes do JDK. Não invente pacotes, classes, métodos, construtores, campos ou enums que não estejam evidentes no contexto fornecido. Evite reflexão por padrão; só use reflexão quando o membro refletido aparecer explicitamente no source_code fornecido e não houver caminho público confiável. Se o teste precisar acessar pacote ou módulo interno, declare isso no cabeçalho jtreg com @modules. Antes de responder, faça uma checagem estática mental: cada teste deve compilar no contexto do OpenJDK e deve ter alta chance de passar no primeiro run. Se houver dúvida entre o baseline WIT e o código atual, priorize o comportamento observável do código atual."
	default:
...
- para OpenJDK/jtreg, o relative_path pode seguir o caminho sugerido, mas o conteúdo Java não deve declarar package; use pacote padrão e cabeçalho jtreg com @test, @summary, @modules quando necessário e @run main;
...
// construirPromptGeracaoHintExcecaoSistema monta o prompt sistêmico para o
// cenário EXCEPTION_HINT: envia ao modelo apenas os tipos de exceção dos
// expaths WIT, sem a estrutura completa de gatilhos e condições de guarda.
func construirPromptGeracaoHintExcecaoSistema(framework string) string {
	base := construirPromptGeracaoSistema(framework)
	return base + " Nesta execução, você receberá apenas os tipos de exceção dos caminhos WIT (sem gatilho, sem condições de guarda e sem evidências). Use esses tipos como hints para derivar casos de teste excepcionais, mas baseie a lógica dos testes exclusivamente no código-fonte dos métodos."
}

// construirPromptGeracaoHintExcecaoUsuario monta o prompt de usuário para o
// cenário EXCEPTION_HINT, enviando tipos de exceção como hints parciais.
func construirPromptGeracaoHintExcecaoUsuario(overview, containerName string, methodsPayload []dominio.AnaliseMetodo, contextoOpcional ...map[string]interface{}) string {
	// Compactar: enviar apenas method + tipos de exceção (sem gatilho/guarda)
	type hintMetodo struct {
		MethodID       string   `json:"method_id"`
		ContainerName  string   `json:"container_name"`
		MethodName     string   `json:"method_name"`
		Signature      string   `json:"signature"`
		SourceCode     string   `json:"source_code"`
		ExceptionHints []string `json:"exception_type_hints"`
	}
	compactado := make([]hintMetodo, 0, len(methodsPayload))
	for _, a := range methodsPayload {
		tipos := make([]string, 0, len(a.CaminhosExcecao))
		seen := map[string]bool{}
		for _, cp := range a.CaminhosExcecao {
			if !seen[cp.TipoExcecao] {
				tipos = append(tipos, cp.TipoExcecao)
				seen[cp.TipoExcecao] = true
			}
		}
		compactado = append(compactado, hintMetodo{
			MethodID:       a.Metodo.IDMetodo,
			ContainerName:  a.Metodo.NomeContainer,
			MethodName:     a.Metodo.NomeMetodo,
			Signature:      a.Metodo.Assinatura,
			SourceCode:     a.Metodo.Origem,
			ExceptionHints: tipos,
		})
	}
	payload, _ := json.MarshalIndent(compactado, "", "  ")
	contextoComum, _ := json.MarshalIndent(selecionarContextoGeracaoPrompt(contextoOpcional...), "", "  ")
	return fmt.Sprintf(`Gere testes Java usando apenas os tipos de exceção como hints — sem estrutura WIT completa.
Return JSON: {"strategy_summary":"...","files":[{"relative_path":"...","content":"...","covered_method_ids":[...],"notes":"..."}]}

Regras obrigatórias (idênticas às do cenário direto, exceto pela presença de hints de tipo de exceção):
- use os exception_type_hints apenas para direcionar casos excepcionais; derive a lógica do código-fonte;
- não invente gatilhos, condições de guarda ou evidências que não apareçam no código;
- só use assertThrows quando o código atual realmente sustentar a exceção indicada pelo hint;
- escreva testes pequenos, concretos e compiláveis.
...
Contexto técnico comum:
%s

Métodos-alvo com hints de exceção:
%s
`, containerName, reduzirVisaoGeralParaGeracao(overview), string(contextoComum), string(payload))
}
