...
func TestPrincipalAjudaListaComandosDeExperimento(t *testing.T) {
	...
	for _, command := range []string{"ingest-witup", "analyze-agentic", "compare-sources", "prepare-phase-two-batch", "submit-openai-batch", "prepare-jdk-global-impact", "evaluate-jdk-global-impact"} {
		if !strings.Contains(output, command) {
			t.Fatalf("expected help output to mention %q, got:\n%s", command, output)
		}
	}
	for _, removedCommand := range []string{"run-phase-two", "preflight-phase-two", "consolidate-first-round", "sample-wit"} {
		if strings.Contains(output, removedCommand) {
			t.Fatalf("help output should not mention removed command %q, got:\n%s", removedCommand, output)
		}
	}
