...
func normalizarRespostaGeracao(payload map[string]interface{}) (string, []dominio.ArquivoTesteGerado) {
	...
	for _, item := range raw {
		...
		files = append(files, dominio.ArquivoTesteGerado{
			...
			AcoesExpath:        normalizarAcoesExpath(entrada["expath_actions"]),
		})
	}
	return resumo, files
}

func normalizarAcoesExpath(raw interface{}) []dominio.AcaoExpath {
	lista, ok := raw.([]interface{})
	if !ok || len(lista) == 0 {
		return nil
	}
	acoes := make([]dominio.AcaoExpath, 0, len(lista))
	for _, item := range lista {
		entrada, ok := item.(map[string]interface{})
		if !ok {
			continue
		}
		id := strings.TrimSpace(fmt.Sprint(entrada["path_id"]))
		acao := strings.TrimSpace(fmt.Sprint(entrada["action"]))
		razao := strings.TrimSpace(fmt.Sprint(entrada["reason"]))
		if id == "" || id == "<nil>" {
			continue
		}
		if acao != "used" && acao != "adapted" && acao != "discarded" {
			continue
		}
		if razao == "<nil>" {
			razao = ""
		}
		acoes = append(acoes, dominio.AcaoExpath{IDCaminho: id, Acao: acao, Razao: razao})
	}
	return acoes
}
